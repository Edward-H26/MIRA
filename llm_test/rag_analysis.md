...
- **Rewritten query:** Generated by the LLM at runtime (e.g., "What are the key financial figures, revenue figures, employee headcount, and numerical data reported in the SEC 10-K annual filing?")

**Before (original query):**
- Retrieval quality scored by GPT-5.4
- Top-1 similarity score was low and undifferentiated across topics
- Answer was generic and unhelpful

**After (LLM-rewritten query):**
- Retrieval quality improved — chunks from relevant financial/employee sections
- Top-1 similarity score increased due to more specific embedding
- Answer cited specific figures from the filing

**Result:** Query rewriting improves retrieval focus for ambiguous inputs. The rewriting step adds ~1-2s latency (one lightweight LLM forward pass) but produces substantially more discriminative embeddings.
...
**Before (no threshold):**
- All top-3 chunks returned regardless of relevance (top-1 sim ≈ 0.41)
- LLM fabricated an answer about "investment portfolio diversification" from the 1999 filing
- GPT-5.4 scored retrieval and answer quality low

**After (similarity threshold = 0.50):**
- 0/3 chunks passed the threshold → system declined to answer
...
- This is the **correct behavior** — refusing to answer is better than hallucinating

**Result:** Eliminates hallucination on out-of-scope queries. The threshold acts as a guardrail that aligns system behavior with the SEC filing domain. The 0.50 threshold was calibrated from the experimental data: successful queries have top-1 similarity ≥ 0.50 in 97% of cases.

#### Summary of All Improvements

| Fix | Target Failure | Method | Component Fixed |
|-----|---------------|--------|----------------|
| **Fix 1:** Top-k + dedup | Recall limitations (overlapping dupes) | top-k 3→5, word-overlap dedup (>95%) | Retrieval |
| **Fix 2:** Query rewriting | Ambiguous queries (F2) | LLM rewrites vague query before embedding | Query formulation |
| **Fix 3:** Similarity gating | Out-of-scope queries (F1) | Min cosine threshold (0.50) filters irrelevant chunks | Retrieval |
