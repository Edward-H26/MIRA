...
**Limited API model evaluation.** Our benchmarking focused on locally deployable open-weight models. We did not systematically benchmark cloud API models (Gemini, GPT-4, Claude) under the same 35-prompt evaluation framework. The estimated ~95% accuracy for Gemini 3 Flash is based on published benchmark performance extrapolated to our domain rather than direct measurement. A complete study would include API models in the same evaluation pipeline.
